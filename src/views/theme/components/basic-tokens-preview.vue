          ...
          <div class="demo-color-box" :style="{ background: dataProxy(color) }">
            ...
            <div class="bg-color-sub">
              <div
                ...
                :key="key"
                :style="{
                  background: tintColor(dataProxy(color), (key + 8) / 10),
                }"
              />
            </div>
          </div>
    ...
    <el-row :gutter="12">
      ...
      <el-col :span="9">
        ...
        >
          ...
          need to be different. There are millions of users. If MySpace layouts
          are chosen well, then you can enhance your profile a great deal.
        </p>
      </el-col>
      <el-col :span="9">
        ...
        >
          ...
          stay in touch with the central office via their notebooks. Computer
          hardware weaves itself through the fabric of our lives.
        </p>
      </el-col>
    </el-row>
  </div>
...
import { ACTION_COMPONECT_SELECT, ACTION_USER_CONFIG_UPDATE } from "@/constant";
import vars from "@/assets/themeConfig/variables.module.scss";

const original = {
  color_primary: vars.primaryColor,
...
export default {
  created() {
    ...
    bus.$on(ACTION_COMPONECT_SELECT, (val) => {
      this.$nextTick(() => {
        ...
        ).filter((el) => el.innerText.toLowerCase() === val);
  ...
  methods: {
    tintColor(a, b) {
      return commonUtil.tintColor(a, b);
    },
    dataProxy(value) {
